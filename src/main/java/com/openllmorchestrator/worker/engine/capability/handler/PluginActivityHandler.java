...
/**
 * CapabilityHandler that represents one plugin/activity by name.
 * Each activity name (e.g. AccessControlPlugin) is implemented by one such handler.
 */
public final class PluginActivityHandler implements CapabilityHandler {
    ...
    public CapabilityResult execute(PluginContext context) {
        return CapabilityResult.builder().capabilityName(activityName).build();
    }
}
