...
public interface CapabilityHandler {
    ...
    /** Unique capability name (must match pipeline config). Return a constant. */
    String name();

    /**
     * Execute the capability. Read from context; write via putOutput/currentPluginOutput.
     * Do not retain context after this method returns.
     */
    CapabilityResult execute(PluginContext context);
}
